...
{
	let blogArray = jsonInput.updates.root.items;
	...
	pageHeader.innerHTML = "Updates | " + thisBlog.title;
...
async function postMarkdownRender(markdownBlock) {
  while (!markdownBlock.hasAttribute('rendered')) {
	...
	for(let i = 0; i < mdBlockImages.length; i++)
	{
		...
		mdBlockImages[i].setAttribute('src', 'updates/'+mdBlockImages[i].getAttribute('src'));
	}
}